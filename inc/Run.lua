...
local Text = "-› اهلا عزيزي [حمودي](tg://user?id="..SUDO_USER..") \n- شكرا لاستخدامك سورس طوكيو \n- أرســل  الان /start\n- لاضهار الاوامر للمطور  المجهزه بالكيبورد️"
...
msg.TheRankCmd = 'Source Dev' 
msg.TheRank = 'Source Dev' 
msg.Rank = 1
...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[-› مرحبا انا آسـمـي ]]..redis:get(max..':NameBot:')..[[

-› اختصاصي حماية المجموعات
-› من السبام و التكرار الخ.. 
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_,"-› تم تحديث الملفات ",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
...
redis:hset(max..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"-› عزيزي تم تغيير Myth بنجاح الان ارسل reload ...🍂")
	end
...
send_msg(msg.chat_id_,"-› تم تثبيت الايدي الان قم برسال معرف المطور 🍃 @UserName ...")
	...
	if msg.text== 'تغير Myth' and msg.sender_user_id_ == SUDO_ID then
